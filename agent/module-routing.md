...
Sub-module agent structures are prohibited unless the project has 3+ bounded contexts declared in root `agent/architecture.md`. The `init-module-agent.sh` script enforces this gate automatically. For projects with fewer than 3 bounded contexts, use the root `agent/` only — sub-module agents waste context without adding value.
...
3. For skills, rules, scripts, and security policy: use the root `agent/` (these are not duplicated per module).
...
4. Changes to the interface between modules follow root `agent/architecture.md`.
5. New shared terms go in root `agent/ubiquitous-language.md`.
6. Cross-module boundary changes require a root-level ADR.

### Module Check Commands

Run the module's own check command (from its `agent/testing-policy.md`) for narrow checks, then the root `./scripts/check.sh` for the full gate.
