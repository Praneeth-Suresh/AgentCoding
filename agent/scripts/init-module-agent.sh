...
# Usage: ./agent/scripts/init-module-agent.sh <module-path> <module-name> <bounded-context>
# Example: ./agent/scripts/init-module-agent.sh modules/billing billing "Plans, invoices, subscription state"
...
ARCH_FILE="${ROOT_DIR}/agent/architecture.md"
[[ -f "$ARCH_FILE" ]] || fail "Root agent/architecture.md not found. Fill project facts first."
...
if [[ $CONTEXT_COUNT -lt 3 ]]; then
  fail "Sub-module agents require 3+ bounded contexts declared in agent/architecture.md (found ${CONTEXT_COUNT}). For smaller projects, use the root agent/ only."
fi
...
sed \
  ...
  "${ROOT_DIR}/agent/templates/module-context.md" > "$AGENT_DIR/module-context.md"
...
Terms specific to this module. Root-level shared terms are in the root \`agent/ubiquitous-language.md\`.
...
printf "  2. Add module to root agent/architecture.md.\n"
printf "  3. Run ./agent/scripts/agent-doctor.sh to verify root is still healthy.\n"
