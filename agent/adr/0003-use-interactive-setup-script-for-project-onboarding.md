...
Add `scripts/setup-project.sh` as the default onboarding interface.

The script asks for a target directory, stack, test runner, hook preference, and optional AI fallback. It copies the control plane, configures `agent/affected-tests.conf`, runs the existing shim and manifest setup scripts, and can invoke a selected headless coding agent when built-in options are insufficient.
